...
- **会話継続（呼び直し不要）** … `FOLLOWUP_ENABLED` / `FOLLOWUP_WINDOW_SEC`（下記）
...
応答を喋り終えた直後、**ウェイクワード（「ずんだもん」）を言わずにそのまま続けて話せる**「窓」が開く。
毎回呼び直す音声 UX のストレスをなくすのが狙い。窓は**無音で開く**（応答ごとのビープは鳴らさない）。
無言のまま `FOLLOWUP_WINDOW_SEC` 秒が過ぎると窓を閉じ、ウェイクワード待ちへ戻る。

| 設定 | 既定 | 意味 |
|---|---|---|
| `FOLLOWUP_ENABLED` | `true` | `false` で従来どおり毎ターン要ウェイクワード |
| `FOLLOWUP_WINDOW_SEC` | `6.0` | 窓の開始待ち。無言でこの秒数経つと閉じる |
| `FOLLOWUP_CLOSE_TEXT` | `また呼んでください` | 窓を閉じる瞬間の読み上げ。**空にすると合図なし**（無音で閉じる） |

- 窓の間に喋れば、その発話が次のターンになり、また窓が開く（沈黙までずっと会話が続く）。
- 閉じる合図は **窓を閉じる瞬間だけ** 読み上げる（応答ごとには鳴らさないので静か）。
- バージイン・ログモード切替・`[[TASK]]` 委譲は窓の中の発話でもそのまま効く。
- スピーカー大音量＋`BARGE_IN_MODE=energy` の環境では、TTS の残響を窓が拾って誤反応しうる。
  気になる場合は `FOLLOWUP_ENABLED=false`、または `BARGE_IN_MODE=wakeword`（既定）で運用する。
