...
起動すると各段階が表示されるので、**どこまで進んでいるか**で切り分けられる。

```
サーバ: http://localhost:8000/api/transcribe
マイク: マイク配列 (Realtek(R) Audio)（2ch）
プッシュトゥトーク: f8 を押している間だけ録音します
終了: <ctrl>+<alt>+q（端末があれば Ctrl+C でも可）
サーバ疎通: OK              ← ここが NG ならサーバ側の問題
* 録音中… (f8 を押している間)  ← 出ないならキーを拾えていない
  送信中… (1.5 秒)            ← 出ないなら音量がしきい値に届いていない
> こんにちは   (0.4 秒)        ← ここまで出て打鍵されないなら注入の問題
```

| どこで止まるか | 原因と対処 |
|---|---|
| `サーバ疎通: OK` が出ない | サーバが起動していないか URL 違い。`server` を確認する |
| キーを押しても `* 録音中…` が出ない | キーが拾えていない。`key` を別のキーにする（`f8` `f9` `scroll_lock` など） |
| `! 音を検出できませんでした` | 音量がしきい値未満。表示された推奨値まで `threshold` を下げる |
| `! 発話が短すぎました` | `min_speech_ms` を下げる（例: `150`） |
| `聞き取れませんでした` | 音は届いているが Whisper が無音と判断。もう少し大きくはっきり話す |
| `> テキスト` は出るが入力されない | 打鍵の問題。下の表を参照 |

**しきい値が分からないときは `debug = true`**（または `--debug`）。録音中の音量がバーで
出るので、話したときの値の 6 割程度を `threshold` にする。
...
> ブラウザ版（Web UI）は `getUserMedia` が**自動ゲイン調整**を掛けるが、このクライアントは
> マイクの生の入力を読む。そのため同じ既定値 `0.015` でも届かないことがある。
